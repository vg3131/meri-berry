...
export function ManageProducePanel() {
  ...
  const [form, setForm] = useState({ name: "", centsPerkKg: "", currencyCode: "AUD" });
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const centsPerkKg = Math.round(Number(form.centsPerkKg) * 100);
    const currencyCode = form.currencyCode.trim().toUpperCase();

    if (!name) { setError("Name is required."); return; }
    if (!Number.isFinite(centsPerkKg) || centsPerkKg <= 0) { setError("Enter a valid rate greater than 0."); return; }
    if (currencyCode.length !== 3) { setError("Currency code must be 3 letters (e.g. AUD)."); return; }

    setIsSubmitting(true);
    setError("");
    setSuccessMessage("");
    try {
      await createFruitType({ name, centsPerkKg, currencyCode });
      setSuccessMessage(`"${name}" added at $${(centsPerkKg / 100).toFixed(2)}/kg.`);
      setForm({ name: "", centsPerkKg: "", currencyCode: "AUD" });
      loadFruitTypes();
    } catch (err) {
    ...
    <section className="panel">
      ...
      <form className="form-grid" onSubmit={handleSubmit}>
        ...
        <label className="field">
          ...
          <input
            ...
            min="0.01"
            step="0.01"
            value={form.centsPerkKg}
            onChange={(e) => setForm({ ...form, centsPerkKg: e.target.value })}
      ...
      <div style={{ marginTop: "1.5rem" }}>
        ...
        {isLoading ? (
          ...
          <p style={{ color: "#596477", fontSize: "0.9rem" }}>
            No produce types yet — add one above.
          </p>
        ) : (
            ...
            {fruitTypes.map((ft) => (
              <section className="result-card" key={ft.id}>
                ...
                <dl className="result-grid">
                  ...
                  <dd>
                    {ft.centsPerkKg != null
                      ? `$${(ft.centsPerkKg / 100).toFixed(2)}/kg`
                      : "—"}
                  </dd>
                  <dt>Currency</dt>
                  <dd>{ft.currencyCode ?? "—"}</dd>
                </dl>
              </section>
            ))}
          </div>
